...
ControlPlane::ControlPlane(sc_module_name nm, pfp::core::PFPObject* parent,
        std::string configfile) : ControlPlaneSIM(nm, parent, configfile) {
  /*sc_spawn threads*/
  ThreadHandles.push_back(sc_spawn(sc_bind(&ControlPlane::CPThread, this, 0)));
  ThreadHandles.push_back(sc_spawn(sc_bind(
        &ControlPlane::command_processing_thread, this)));
}
...
void ControlPlane::CPThread(std::size_t thread_id) {
...
// The ControlPlane uses the CommandProcessor and ResultProcessor
// interfaces to notify the debugger about changes

...
void AddTableEntry::execute(pfp::core::PFPObject& module) {
  ...
  if (rc != bm::MatchErrorCode::SUCCESS) {
    ...
    // Update this to add table entry into NVM Match Table
    NVMMatchTable& match_table = static_cast<NVMMatchTable&>(module);

    npulog(profile, std::cout << "Sending TableEntryAdded message to NVM: "
